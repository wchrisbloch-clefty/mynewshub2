  ...
  if (activeNote) {
    ...
    return (
      <div style={{ padding: '24px 28px 80px', maxWidth: 760, margin: '0 auto' }}>
        <div onClick={() => setActiveNote(null)} style={{ fontSize: 11, color: 'var(--subtle)', cursor: 'pointer', marginBottom: 20 }}>← Vault</div>
    ...
    <div style={{ padding: isMobile ? '16px 16px 60px' : '24px 28px 60px', maxWidth: 900, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 28 }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: 'var(--subtle)' }}>{notes.length} notes · synthesized insights, cross-references, and breakthroughs</div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 20 }}>
          {['all', ...allTags].map(t => (
            <div key={t} onClick={() => setFilterTag(t)}
              style={{ padding: '4px 12px', fontSize: 9, letterSpacing: 1.5, textTransform: 'uppercase', border: `1px solid ${filterTag === t ? '#ffcc44' : 'var(--bord2)'}`, color: filterTag === t ? '#ffcc44' : 'var(--subtle)', borderRadius: 20, cursor: 'pointer', background: filterTag === t ? '#ffcc4418' : 'transparent' }}>
              {t}
            </div>
          ))}
        </div>
