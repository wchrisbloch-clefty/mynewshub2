...
export default function TopBar() {
  const { activeModule, setActiveModule, graph, projects, chatOpen, setChatOpen, searchQuery, setSearchQuery, isMobile, theme, toggleTheme } = useApp();
  ...
  const themeBtn = (
    <div
      ...
      style={{
        ...
        cursor: 'pointer', fontSize: 15, transition: 'all 0.2s', flexShrink: 0,
      }}
  ...
  if (isMobile) {
    return (
      <header style={{
        height: 56, borderBottom: '1px solid var(--bord2)',
        background: 'var(--bg)', display: 'flex', alignItems: 'center',
        padding: '0 16px', gap: 10, position: 'relative', zIndex: 20, flexShrink: 0,
      }}>
        {mobileSearchOpen ? (
          <div style={{
            position: 'absolute', top: 0, left: 0, right: 0, height: 56,
            background: 'var(--bg)', borderBottom: '1px solid var(--bord2)',
            display: 'flex', alignItems: 'center', padding: '0 16px', gap: 10, zIndex: 30,
          }}>
            <input
              ref={mobileSearchRef}
              value={searchQuery}
              ...
              style={{
                flex: 1, background: 'var(--surface)', border: `1px solid ${accent}60`,
                borderRadius: 10, padding: '8px 14px', color: 'var(--text-b)',
                fontSize: 13, outline: 'none', fontFamily: 'inherit',
              }}
            />
            <div onClick={() => { setMobileSearchOpen(false); setSearchQuery(''); }}
              style={{ fontSize: 14, color: 'var(--subtle)', cursor: 'pointer', padding: '4px 6px', flexShrink: 0 }}>✕</div>
            {filtered.length > 0 && (
              <div style={{
        ...
        <div onClick={() => setMobileSearchOpen(true)}
          style={{ width: 34, height: 34, borderRadius: '50%', background: 'var(--surface)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: 15, color: 'var(--subtle)', flexShrink: 0 }}>⌕</div>

        <div onClick={() => setChatOpen(!chatOpen)}
          style={{ width: 34, height: 34, borderRadius: '50%', background: chatOpen ? 'linear-gradient(135deg, #00FFB2, #6366F1)' : 'var(--surface)', border: `1px solid ${chatOpen ? 'transparent' : 'var(--border)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: 14, transition: 'all 0.2s', flexShrink: 0 }}>{chatOpen ? '✕' : '💬'}</div>
      </header>
    );
  }
  ...
  return (
    <header style={{
      height: 56, borderBottom: '1px solid var(--bord2)',
      background: 'var(--bg)', display: 'flex', alignItems: 'center',
      padding: '0 20px', gap: 16, position: 'relative', zIndex: 20, flexShrink: 0,
    }}>
      ...
      <div style={{ flex: 1, maxWidth: 380, position: 'relative' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--surface)', border: `1px solid ${searchFocused ? accent + '60' : 'var(--border)'}`, borderRadius: 10, padding: '7px 14px', transition: 'border-color 0.15s' }}>
          ...
          />
          {searchQuery && <span onClick={() => setSearchQuery('')} style={{ fontSize: 10, color: 'var(--subtle)', cursor: 'pointer' }}>✕</span>}
        </div>
        {filtered.length > 0 && searchFocused && (
          <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: 4, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden', zIndex: 100 }}>
            {filtered.map((s, i) => (
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
        <div style={{ fontSize: 10, color: 'var(--subtle)', padding: '4px 10px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 20 }}>
      ...
      <div onClick={() => setChatOpen(!chatOpen)}
        style={{ width: 34, height: 34, borderRadius: '50%', background: chatOpen ? 'linear-gradient(135deg, #00FFB2, #6366F1)' : 'var(--surface)', border: `1px solid ${chatOpen ? 'transparent' : 'var(--border)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: 14, transition: 'all 0.2s', flexShrink: 0 }}
