  ...
  return (
    <div style={{ padding: isMobile ? '16px 16px 60px' : '24px 28px 60px', maxWidth: 960, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <div style={{ fontSize: 24, fontWeight: 800, color: '#fff', fontFamily: "'Fraunces', serif", letterSpacing: -0.5, marginBottom: 4 }}>{greeting}, CB.</div>
        <div style={{ fontSize: 12, color: '#445' }}>Your intelligence hub is ready. {topics.length > 0 ? `${topics.length} topics tracked, ${activeProjects.length} projects active.` : 'Start by adding a book or topic.'}</div>
      </div>
        ...
        {statCards.map(s => (
          <Card key={s.label} color={s.accent} style={{ padding: '14px 16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
              ...
              <div style={{ fontSize: 9, color: '#334', letterSpacing: 1 }}>{s.sub}</div>
            </div>
            <div style={{ fontSize: 22, fontWeight: 800, color: s.accent, marginBottom: 2, fontFamily: "'Fraunces', serif" }}>{s.value}</div>
            <div style={{ fontSize: 10, color: '#445', letterSpacing: 0.5 }}>{s.label}</div>
          </Card>
        ))}
      </div>
        ...
        <Card color="#00FFB2" style={{ padding: '18px 18px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            <div>
              ...
              <div style={{ fontSize: 11, color: '#445', marginTop: -4 }}>CB-style — big picture first</div>
            </div>
            {!briefGenerated && (
          ...
          {briefLoading && (
            <div style={{ padding: '12px 0' }}>
              <div style={{ fontSize: 10, color: '#334', marginBottom: 10 }}>Generating your intelligence brief...</div>
              <ThinkingDots color="#00FFB2" />
            </div>
          )}
          {briefGenerated && brief && <MD text={brief} color="#00FFB2" />}
          {!briefGenerated && !briefLoading && (
            <div style={{ textAlign: 'center', padding: '20px 16px', color: '#334' }}>
        ...
        <Card color="#ff8844" style={{ padding: '18px 18px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {activeProjects.slice(0, 3).map(p => {
              ...
              return (
                <div key={p.id} onClick={() => setActiveModule('projects')} style={{ padding: '10px 12px', background: '#0c0c18', borderRadius: 8, border: '1px solid #1e2a38', cursor: 'pointer' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: '#fff' }}>{p.emoji} {p.title}</div>
            ...
            {activeProjects.length === 0 && <div style={{ fontSize: 11, color: '#334', textAlign: 'center', padding: 16 }}>No active projects. Add one →</div>}
          </div>
        </Card>
      </div>
          ...
          {BLUE_OCEAN_SIGNALS.map((s, i) => (
            <Card key={i} color={s.urgency === 'high' ? '#00FFB2' : '#6366F1'} style={{ padding: '14px 16px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                ...
                <Badge color={s.urgency === 'high' ? '#00FFB2' : '#6366F1'}>{s.urgency}</Badge>
              </div>
              <div style={{ fontSize: 12, fontWeight: 700, color: '#fff', marginBottom: 6, lineHeight: 1.4 }}>{s.title}</div>
              <div style={{ fontSize: 10, color: '#445', lineHeight: 1.65 }}>{s.insight}</div>
            </Card>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 20 }}>
        ...
        <div>
          ...
          {recentSessions.length === 0 && <div style={{ fontSize: 11, color: '#334', padding: '16px 0' }}>No sessions yet. Start learning →</div>}
          {recentSessions.map((s, i) => (
            <div key={i} onClick={() => setActiveModule('learn')} style={{ padding: '10px 14px', background: '#0c0c18', border: '1px solid #1e2a38', borderRadius: 8, marginBottom: 7, cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <div style={{ fontSize: 12, fontWeight: 600, color: '#fff' }}>{s.title}</div>
                <div style={{ fontSize: 9, color: '#334', marginTop: 2 }}>{timeAgo(s.date)} · {s.durationMin}min</div>
              </div>
        ...
        <div>
          ...
          {recentNotes.length === 0 && <div style={{ fontSize: 11, color: '#334', padding: '16px 0' }}>No notes yet. Save insights →</div>}
          {recentNotes.map(n => (
            <div key={n.id} onClick={() => setActiveModule('vault')} style={{ padding: '10px 14px', background: '#0c0c18', border: `1px solid ${n.color}20`, borderRadius: 8, marginBottom: 7, cursor: 'pointer' }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: '#fff', marginBottom: 4 }}>{n.title}</div>
              <div style={{ fontSize: 10, color: '#445', lineHeight: 1.55, overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{n.content}</div>
            </div>
          ))}
        </div>
      </div>
