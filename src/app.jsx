...
/* ═══════════════════════════════════════════
   v16 ADDITIONS — Mobile overhaul + editorial strips
═══════════════════════════════════════════ */
...
@media (max-width:640px){
  ...
  .hero-lead-img{aspect-ratio:16/10;max-height:280px;}
...
export default function App() {
  ...
  const TodayPage = () => {
    ...
    return (
      <div className="page">
        {/* v16: Right Now — live glance strip (urgent + scores + big moves) */}
        <RightNowStrip
        ...
        {/* v16: Following — tracked entities row (Yahoo Sports "My Teams" pattern) */}
        <FollowingStrip
          following={following}
          arts={arts}
          marketData={marketData}
          onFilter={(ent) => {
            // Tap an entity → jump to General feed scoped to its match terms.
            // We reuse the existing activeKw mechanism so the feed naturally filters.
            if (ent.match && ent.match.length) setActiveKw(ent.match[0]);
            // Route to the most relevant category based on the entity kind
        ...
        {/* v16: Trending carousel moved inline from sidebar — above-the-fold */}
