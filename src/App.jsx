...
const PODCAST_FEEDS = [
{ name:'Joe Rogan Experience', host:'Joe Rogan', url:'https://feeds.megaphone.fm/GL
{ name:'Ben Shapiro Show', host:'Ben Shapiro', url:'https://feeds.megaphone.fm/BV
{ name:'Tucker Carlson Show', host:'Tucker Carlson', url:'https://feeds.megaphone.fm/RS
{ name:'Candace', host:'Candace Owens', url:'https://feeds.megaphone.fm/ca
{ name:'Morning Wire', host:'Daily Wire', url:'https://feeds.megaphone.fm/BV
{ name:'All-In Podcast', host:'Chamath & Besties', url:'https://allinchamathjason.lib
{ name:'Flagrant', host:'Andrew Schulz', url:'https://feeds.megaphone.fm/AP
];
const DEFAULT_KW = {
general: ['Houston','Texas','Trump','Congress','White House','geopolitical','AI','tech','I
sports: ['Texans','Rockets','Astros','Braves','Kentucky','Clemson','NFL','MLB','NBA','CFB
business: ['energy','oil','gas','data center','ERCOT','LNG','power grid','onshoring','AI','
finance: ['investing','real estate','stock market','interest rates','Fed','inflation','cry
bloom: ['Bloom Energy','fuel cell','hydrogen','microgrid','distributed power','data cent
...
{ name:'Chron.com', url:'https://www.chron.com/rss/feed/News-270.php',
{ name:'Morning Brew', url:'https://www.morningbrew.com/feed',
{ name:'Morning Wire', url:'https://feeds.megaphone.fm/BVDWV8747925072',
],
sports: [
{ name:'ESPN NFL', url:'https://www.espn.com/espn/rss/nfl/news',
{ name:'ESPN NBA', url:'https://www.espn.com/espn/rss/nba/news',
{ name:'ESPN MLB', url:'https://www.espn.com/espn/rss/mlb/news',
{ name:'ESPN CFB', url:'https://www.espn.com/espn/rss/ncf/news',
...
{ name:'CBS Sports CFB', url:'https://www.cbssports.com/rss/headlines/college-footb
{ name:'CBS Sports CBB', url:'https://www.cbssports.com/rss/headlines/college-baske
{ name:'Pro Football Talk', url:'https://www.nbcsports.com/profootballtalk.rss',
{ name:'Bleacher Report', url:'https://feeds.bleacherreport.com/articles/feed',
{ name:'247Sports', url:'https://247sports.com/Page/College-Sports-News-and-Re
{ name:'Kentucky Sports Radio',url:'https://kentuckysportsradio.com/feed/',
{ name:'On3 Recruiting', url:'https://www.on3.com/feed/',
{ name:'The Spun', url:'https://thespun.com/.rss/full/',
],
business: [
{ name:'Reuters Business', url:'https://feeds.reuters.com/reuters/businessNews',
{ name:'CNBC Energy', url:'https://www.cnbc.com/id/10000664/device/rss/rss.htm
...
{ name:'Rigzone', url:'https://www.rigzone.com/news/rss/rigzone_latest.asp
...
{ name:'CNBC Tech', url:'https://www.cnbc.com/id/19854910/device/rss/rss.htm
],
finance: [
{ name:'MarketWatch', url:'https://feeds.marketwatch.com/marketwatch/topstories/',
{ name:'Yahoo Finance', url:'https://finance.yahoo.com/news/rssindex',
{ name:'Kiplinger', url:'https://www.kiplinger.com/rss/all',
{ name:'Motley Fool', url:'https://www.fool.com/feeds/index.aspx',
{ name:'Investopedia', url:'https://www.investopedia.com/feedbuilder/feed/getfeed/?feedN
{ name:'BiggerPockets', url:'https://www.biggerpockets.com/blog/feed',
...
general: {
twitter: ['@Bloomberg','@Reuters','@WSJ','@FoxNews','@CNN','@CNBC','@BBCWorld','@DailyW
linkedin: [],
instagram: [],
youtube: ['AssociatedPress','CNBC','BloombergTelevision'],
},
sports: {
twitter: ['@HoustonTexans','@HoustonRockets','@astros','@Braves','@KentuckyMBB','@UKFoo
linkedin: [],
instagram: ['@houstontexans','@houstonrockets','@astros','@braves','@kentuckymbb','@ukfoo
youtube: ['ESPN','NFL','NBA','MLB'],
},
business: {
twitter: ['@OilandGasJnl','@HartEnergy','@UtilityDive','@EENewsUpdates','@POWERmagazine
linkedin: ['Marathon Petroleum','ExxonMobil','Chevron','Shell','NRG Energy','Bloom Energ
instagram: [],
youtube: ['bloomenergy','Bloomberg','CNBC'],
},
finance: {
twitter: ['@MarketWatch','@YahooFinance','@Kiplinger','@MotleyFool','@Investopedia','@B
...
const SCORE_TEAMS = [
{ team:'Texans', sport:'football', league:'nfl', match:'Houst
{ team:'Rockets', sport:'basketball', league:'nba', match:'Houst
{ team:'Astros', sport:'baseball', league:'mlb', match:'Houst
{ team:'Braves', sport:'baseball', league:'mlb', match:'Atlan
{ team:'UK Basketball',sport:'basketball', league:'mens-college-basketball', match:'Kentu
{ team:'UK Football', sport:'football', league:'college-football', match:'Kentu
{ team:'Clemson FB', sport:'football', league:'college-football', match:'Clems
];
emoji
emoji
emoji
emoji
const LEAGUES = [
{ key:'nfl', label:'NFL', sport:'football', league:'nfl', { key:'nba', label:'NBA', sport:'basketball', league:'nba', { key:'mlb', label:'MLB', sport:'baseball', league:'mlb', { key:'cfb', label:'College FB',sport:'football', league:'college-football', { key:'cbb', label:'College BB',sport:'basketball', league:'mens-college-basketball', emoji
];
const SK = 'v21_';
...
const WX_CODES = {0:'Clear',1:'Mostly Clear',2:'Partly Cloudy',3:'Overcast',45:'Foggy',48:'I
const WX_EMOJI = {0:' ',1:' ',2:' ',3:' ',45:' ',48:' ',51:' ',53:' ',55:' ',61:'
// ─── STORAGE ─────────────────────────────────────────────────────────────────
function ld(k, d) {
try {
...
if (i.enclosure?.link && /\.(jpg|jpeg|png|webp|gif)/i.test(i.enclosure.link)) return const html = i.content || i.description || '';
...
return items.map(i => {
const descRaw = i.querySelector('description')?.textContent || i.querySelector('summary')
const desc = descRaw.replace(/<[^>]*>/g,'').replace(/&amp;/g,'&').replace(/&nbsp;/g,' ').
return {
...
if (r.ok) { const items = parseXML(await r.text()); if (items.length) return {items, reas
} catch {}
try {
if (r.ok) {
const r = await fetchWithTimeout(`https://api.rss2json.com/v1/api.json?rss_url=${encodeUR
const d = await r.json();
if (d.items?.length > 0) return {
items: d.items.map(i => ({
title:(i.title||'').trim(), link:i.link,
desc:(i.description||i.content||'').replace(/<[^>]*>/g,'').replace(/&amp;/g,'&').re
...
try {
const r = await fetchWithTimeout(`https://api.allorigins.win/get?url=${encodeURIComponent
if (r.ok) { const d = await r.json(); if (d.contents) { const items = parseXML(d.contents
} catch {}
try {
...
const h=dt.getHours()%12||12, m=String(dt.getMinutes()).padStart(2,'0'), ampm=dt.getHours
...
try {
const r = await fetchWithTimeout(`https://api.open-meteo.com/v1/forecast?latitude=${city.
const d = await r.json(), c = d.current;
return {name:city.name, slug:city.slug, temp:Math.round(c.temperature_2m), code:c.weather
} catch { return null; }
}
...
if (r.ok) { const d=await r.json(); if (typeof d.price==='number') return {price:d.price,
} catch {}
try {
const url=`https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(sym)}?i
const r=await fetchWithTimeout(`https://api.allorigins.win/get?url=${encodeURIComponent(u
...
try {
const r = await fetchWithTimeout(`https://site.api.espn.com/apis/site/v2/sports/${sport}/
if (!r.ok) return [];
const d = await r.json();
...
if (!game) return null;
const txt=((game.homeName||'')+' '+(game.awayName||'')+' '+(game.short||'')+' '+(game.name|
...
case 'linkedin': return `https://www.linkedin.com/search/results/all/?keywords=${encodeU
...
if (r.status===500&&/API_KEY/i.test(detail)) return {summary:'',error:'No AI provider c
...
<div><span className="takeaway-head">{m[2].trim()}</span><span className="takeawa
</div>
);
})}
</div>
return <div key={i} className="takeaway-item"><div style={{fontSize:'12px',color:'var
);
}
// ─── SOURCE URL MAP ───────────────────────────────────────────────────────────
const SOURCE_URLS = {
...
'CBS Sports NFL':'https://www.cbssports.com/nfl','CBS Sports NBA':'https://www.cbssports.co
'CBS Sports MLB':'https://www.cbssports.com/mlb','CBS Sports CFB':'https://www.cbssports.co
'CBS Sports CBB':'https://www.cbssports.com/college-basketball',
'Pro Football Talk':'https://profootballtalk.nbcsports.com','Bleacher Report':'https://blea
'247Sports':'https://247sports.com','Kentucky Sports Radio':'https://kentuckysportsradio.co
'On3 Recruiting':'https://www.on3.com','The Spun':'https://thespun.com',
'Reuters Business':'https://www.reuters.com/business','CNBC Energy':'https://www.cnbc.com/e
'Oilprice.com':'https://oilprice.com','Utility Dive':'https://www.utilitydive.com',
'Data Center Dynamics':'https://www.datacenterdynamics.com','Power Magazine':'https://www.p
'Rigzone':'https://www.rigzone.com','MIT Tech Review':'https://www.technologyreview.com',
'AI News':'https://artificialintelligence-news.com','Canary Media':'https://www.canarymedia
'The Guardian Business':'https://www.theguardian.com/business','CNBC Tech':'https://www.cnb
...
'Investopedia':'https://www.investopedia.com','BiggerPockets':'https://www.biggerpockets.co
...
.wx-city{font-weight:600;color:rgba(220,220,240,0.75);font-size:10px;text-transform:uppercase
...
.ticker-row{display:flex;gap:4px;align-items:center;overflow-x:auto;scrollbar-width:none;flex
...
.ticker-sym{font-size:11px;font-weight:700;font-variant-numeric:tabular-nums;letter-spacing:-
...
padding:2px 10px;white-space:nowrap;flex-shrink:0;border-right:1px solid rgba(255,255,255,0
}
...
.logo-tag{font-size:7.5px;color:var(--text3);letter-spacing:0.14em;text-transform:uppercase;f
.nav-tabs{
...
.search-input:focus{outline:none;border-color:var(--accent);box-shadow:0 0 0 3px var(--accent
.nav-btn{
...
.refresh-btn{
background:var(--accent);border:none;color:#fff;
border-radius:8px;padding:7px 16px;font-size:12px;font-weight:600;cursor:pointer;font-famil
}
/* ═══════════════════════════════════════════
GHOST FEED CARD
...
.fc-date{font-size:11px;color:var(--text3);font-variant-numeric:tabular-nums;margin-left:auto
.fc-topic{
...
display:flex;align-items:center;justify-content:center;font-size:22px;background:var(--surf
}
...
font-family:inherit;font-weight:600;transition:all 0.12s;display:flex;align-items:center;ga
}
...
.fc-summary-lbl{font-size:9px;font-weight:700;color:#7c3aed;text-transform:uppercase;letter-s
.fc-summary-text{font-size:12px;color:var(--text2);line-height:1.6;}
.fc-takeaways{background:var(--surface2);border-radius:8px;padding:12px 14px;border-left:2px
.fc-takeaways-lbl{font-size:9px;font-weight:700;color:#4f46e5;text-transform:uppercase;letter
.takeaways-list{display:flex;flex-direction:column;gap:8px;}
.takeaway-item{display:flex;gap:8px;align-items:flex-start;font-size:12px;line-height:1.5;col
.takeaway-num{font-size:14px;font-weight:900;color:#4f46e5;min-width:18px;flex-shrink:0;line-
...
.fc-disc-lbl{font-size:9px;font-weight:700;color:#0284c7;text-transform:uppercase;letter-spac
.fc-disc-item{display:flex;align-items:center;gap:8px;padding:5px 0;text-decoration:none;colo
.fc-disc-item:hover{color:#0284c7;}
.fc-disc-platform{font-size:8px;font-weight:800;border-radius:3px;padding:2px 5px;color:#fff;
...
.fc-disc-title{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;f
.fc-disc-stats{margin-left:auto;font-size:10px;color:var(--text3);white-space:nowrap;flex-shr
/* Related sources */
...
border:1px solid var(--border);color:var(--text3);background:none;font-family:inherit;trans
}
...
.gs-clear{background:none;border:none;color:var(--accent);cursor:pointer;font-size:10px;font-
...
.gs-filter-x{background:none;border:none;color:var(--accent);cursor:pointer;font-size:13px;li
/* Keyword chips */
...
display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;margin
}
.trend-src{font-size:10px;color:var(--text3);display:flex;align-items:center;gap:4px;flex-wra
.trend-cat-badge{
...
.h-green{background:#16a34a;}.h-yellow{background:#d97706;}.h-red{background:#dc2626;}.h-gray
.src-name{font-size:11px;font-weight:500;color:var(--text);flex:1;white-space:nowrap;overflow
...
.sb-box-title{font-size:11px;font-weight:700;color:var(--text2);text-transform:uppercase;lett
.sb-box-sub{font-size:9px;color:var(--text3);text-transform:uppercase;letter-spacing:0.05em;}
.sb-empty{padding:12px 14px;font-size:11px;color:var(--text3);text-align:center;font-style:it
...
.sb-league-meta{margin-left:auto;display:flex;gap:5px;align-items:center;font-size:10px;color
.sb-league-fav{background:#fef3c7;color:#92400e;border-radius:6px;padding:1px 5px;font-weight
...
.sb-game-row{display:flex;align-items:center;justify-content:space-between;gap:6px;padding:1p
...
.sb-abbr{font-size:11px;font-weight:600;color:var(--text);white-space:nowrap;overflow:hidden;
.sb-num{font-size:13px;font-weight:800;color:var(--text);flex-shrink:0;min-width:22px;text-al
.sb-num.winner{color:var(--green);}
.sb-num.loser{color:var(--text3);font-weight:500;}
.sb-status{font-size:9px;margin-top:4px;text-transform:uppercase;letter-spacing:0.04em;displa
...
.briefing-date{font-size:10px;color:var(--text3);text-transform:uppercase;letter-spacing:0.06
.briefing-refresh{
...
.hero-dots{position:absolute;bottom:12px;left:50%;transform:translateX(-50%);display:flex;gap
.hero-dot{
...
display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;font-w
.hero-lead-meta{display:flex;align-items:center;gap:5px;font-size:11px;color:var(--text3);}
.hero-lead-source{font-weight:700;color:var(--text2);text-transform:uppercase;font-size:10px;
.hero-side{
background:var(--surface);border-radius:14px;border:1px solid var(--border);
...
.today-see-all{font-size:10px;background:none;border:none;cursor:pointer;font-family:inherit;
...
.today-thumb-ph{width:48px;height:36px;border-radius:4px;flex-shrink:0;display:flex;align-ite
.today-item-body{flex:1;min-width:0;}
.today-item-title{font-size:11px;font-weight:600;color:var(--text);line-height:1.35;margin-bo
...
.bloom-strip-item{padding:10px 14px;border-right:1px solid var(--border2);cursor:pointer;tran
.bloom-strip-item:last-child{border-right:none;}
.bloom-strip-item:hover{background:var(--surface2);}
.bloom-strip-title{font-size:11px;font-weight:600;color:var(--text);line-height:1.35;margin-b
...
.fin-header-top{display:flex;justify-content:space-between;align-items:flex-start;margin-bott
.fin-header-title{font-size:17px;font-weight:800;color:var(--text);letter-spacing:-0.4px;}
.fin-header-sub{font-size:11px;color:var(--text2);margin-top:3px;display:flex;align-items:cen
.fin-status-dot{width:7px;height:7px;border-radius:50%;display:inline-block;}
.fin-refresh{
background:var(--surface2);border:1px solid var(--border);color:var(--text2);
border-radius:6px;padding:5px 11px;font-size:11px;font-weight:600;cursor:pointer;font-famil
...
.fin-index{background:var(--surface2);border-radius:8px;padding:10px 14px;border-left:3px sol
.fin-index.up{border-left-color:var(--green);}
.fin-index.down{border-left-color:var(--red);}
.fin-index-label{font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;l
.fin-index-price{font-size:20px;font-weight:800;color:var(--text);font-variant-numeric:tabula
.fin-index-chg{font-size:11px;font-weight:600;margin-top:3px;display:flex;gap:5px;align-items
...
.fin-watchlist,.fin-news{background:var(--surface);border:1px solid var(--border);border-radi
.fin-section-head{display:flex;justify-content:space-between;align-items:center;padding:11px
.fin-section-title{font-size:13px;font-weight:800;color:var(--text);letter-spacing:-0.2px;}
.fin-news{padding:14px;}
.fin-news .fin-section-head{margin:-14px -14px 10px;border-radius:10px 10px 0 0;background:va
.fin-table{width:100%;border-collapse:collapse;font-variant-numeric:tabular-nums;}
.fin-table thead th{background:var(--surface2);font-size:9px;font-weight:700;color:var(--text
.fin-table tbody tr{cursor:pointer;transition:background 0.08s;border-bottom:1px solid var(--
...
.fin-sym{font-weight:800;font-family:'SF Mono','Cascadia Code','Consolas',monospace;letter-sp
.fin-name{color:var(--text2);font-size:11px;}
.fin-px{font-family:'SF Mono','Cascadia Code','Consolas',monospace;text-align:right;font-weig
.fin-up{color:var(--green);}
.fin-down{color:var(--red);}
.fin-pct-pill{background:rgba(22,163,74,0.1);border-radius:4px;padding:2px 5px;font-size:11px
...
.social-block-head{display:flex;align-items:center;justify-content:space-between;margin-botto
.social-block-title{font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercas
.social-block-sub{font-size:10px;color:var(--text3);}
.social-platforms{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:1
...
.social-plat-icon{width:16px;height:16px;border-radius:3px;display:flex;align-items:center;ju
...
/* Social Page */
.social-page-header{display:flex;align-items:center;justify-content:space-between;margin-bott
...
.social-page-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:1
.social-cat-block{background:var(--surface);border-radius:var(--radius);border:1px solid var(
.social-cat-head{display:flex;align-items:center;justify-content:space-between;padding:11px 1
...
.pod-title{font-size:13px;font-weight:700;color:var(--text);line-height:1.35;margin-bottom:4p
.pod-title:hover{color:#e11d48;}
.pod-meta{font-size:10px;color:var(--text3);display:flex;gap:8px;flex-wrap:wrap;}
.pod-desc{font-size:11px;color:var(--text2);line-height:1.5;margin-top:6px;display:-webkit-bo
.pod-actions{display:flex;gap:6px;flex-wrap:wrap;}
.pod-btn{border:1px solid var(--border);border-radius:6px;padding:4px 10px;font-size:10px;cur
.pod-btn:hover{border-color:#e11d48;color:#e11d48;}
.pod-btn.saved{border-color:var(--amber);color:var(--amber);background:#fffbeb;}
.pod-shows{background:var(--surface);border-radius:10px;border:1px solid var(--border);paddin
.pod-show-item{display:flex;align-items:center;gap:8px;padding:8px 0;border-bottom:1px solid
...
.pod-show-ep{font-size:10px;color:var(--text3);margin-top:1px;white-space:nowrap;overflow:hid
.pod-show-dot{width:5px;height:5px;border-radius:50%;background:#e11d48;flex-shrink:0;margin-
.saved-empty{text-align:center;padding:80px 20px;}
/* ═══════════════════════════════════════════
CUSTOMIZE PANEL
═══════════════════════════════════════════ */
.cp-overlay{position:fixed;inset:0;background:rgba(0,0,0,0.45);z-index:600;display:flex;align
.cp-panel{background:var(--surface);border-radius:16px;width:100%;max-width:520px;max-height:
.cp-head{padding:16px 20px;border-bottom:1px solid var(--border);display:flex;justify-content
.cp-title{font-size:14px;font-weight:700;color:var(--text);}
.cp-x{background:none;border:none;font-size:18px;cursor:pointer;color:var(--text3);line-heigh
.cp-body{padding:16px 20px;display:flex;flex-direction:column;gap:20px;}
.cp-sec{}
.cp-lbl{font-size:9px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spac
...
.cp-cat-tab{background:var(--surface2);border:1px solid var(--border);border-radius:6px;paddi
.cp-cat-tab.active{background:var(--accent);color:#fff;border-color:var(--accent);}
.cp-sec-tabs{display:flex;gap:4px;flex-wrap:wrap;margin-bottom:14px;padding-bottom:10px;borde
.cp-sec-tab{background:var(--surface2);border:1px solid var(--border);border-radius:6px;paddi
...
.cp-chip{display:inline-flex;align-items:center;gap:3px;border-radius:20px;padding:3px .cp-chip-kw{background:var(--accent-bg);color:var(--accent);}
...
10px;f
.cp-chip-x{background:none;border:none;cursor:pointer;font-size:12px;opacity:0.5;line-height:
.cp-chip-x:hover{opacity:1;}
.cp-add{display:flex;gap:6px;}
.cp-input{flex:1;border:1px solid var(--border);border-radius:6px;padding:6px 10px;font-size:
.cp-input:focus{outline:none;border-color:var(--accent);}
.cp-btn{background:var(--accent);border:none;color:#fff;border-radius:6px;padding:6px 12px;fo
.cp-btn-red{background:var(--red);}
.cp-src-row{display:flex;align-items:center;gap:8px;padding:7px 0;border-bottom:1px solid var
.cp-src-row:last-child{border-bottom:none;}
.cp-health{width:6px;height:6px;border-radius:50%;flex-shrink:0;}
.cp-h-green{background:var(--green);}.cp-h-yellow{background:var(--amber);}.cp-h-red{backgrou
.cp-src-name{flex:1;font-size:12px;color:var(--text);}
.cp-src-count{font-size:10px;color:var(--text3);}
.cp-test-btn{background:var(--surface2);border:1px solid var(--border);border-radius:5px;padd
.cp-tog{width:30px;height:17px;border-radius:9px;border:none;cursor:pointer;position:relative
.cp-tog.on{background:var(--accent);}.cp-tog.off{background:#cbd5e1;}
.cp-tog::after{content:'';width:13px;height:13px;background:#fff;border-radius:50%;position:a
.cp-tog.on::after{left:15px;}.cp-tog.off::after{left:2px;}
.cp-del{background:none;border:none;color:var(--text3);cursor:pointer;font-size:13px;padding:
...
.cp-add-src{background:var(--surface2);border:1px solid var(--border);border-radius:8px;paddi
.cp-add-src-title{font-size:10px;font-weight:600;color:var(--text2);margin-bottom:2px;}
.cp-input-sm{border:1px solid var(--border);border-radius:6px;padding:5px 9px;font-size:11px;
.cp-input-sm:focus{outline:none;border-color:var(--accent);}
.cp-save{width:100%;background:var(--text);border:none;color:var(--bg);border-radius:8px;padd
.cp-save:hover{opacity:0.88;}
.cp-legend{display:flex;gap:10px;font-size:10px;color:var(--text3);margin-bottom:8px;flex-wra
...
.cp-plat-tab{background:var(--surface2);border:1px solid var(--border);border-radius:6px;padd
...
.src-footer-label{font-size:9px;font-weight:700;color:var(--text3);text-transform:uppercase;l
.src-footer-links{display:flex;flex-wrap:wrap;gap:2px 0;align-items:center;}
.src-footer-link{font-size:10px;color:var(--text3);text-decoration:none;padding:2px 7px;borde
...
.follow-card:hover{transform:none;border:none;background:var(--surface2);padding:6px 8px;marg
.trending-card{background:transparent;border:none;padding:8px 0;}
.trending-card:hover{transform:none;border:none;background:var(--surface2);padding:8px 10px;m
/* Podcast cards — Ghost */
...
.mobile-header{display:flex;align-items:center;justify-content:space-between;padding:0 .mobile-logo{font-size:17px;font-weight:900;color:var(--text);letter-spacing:-0.8px;line-heig
.mobile-logo span{color:var(--accent);}
.mobile-logo-sub{font-size:8px;color:var(--text3);letter-spacing:0.14em;text-transform:upperc
...
.menu-sheet-grab{width:36px;height:4px;border-radius:2px;background:var(--text4);margin:8px a
.menu-sheet-head{padding:0 20px 10px;font-size:11px;color:var(--text3);text-transform:upperca
.menu-sheet-item{
...
.rn-pulse{width:6px;height:6px;border-radius:50%;background:var(--red);animation:pulse-badge
...
.follow-edit{background:none;border:none;color:var(--text3);font-size:11px;font-weight:600;cu
...
.trending-card-num{font-size:14px;font-weight:900;color:var(--text4);font-variant-numeric:tab
.trending-card-title{
...
.fc-read-link{font-size:11px;margin-left:auto;min-height:44px;display:flex;align-items:cent
/* Morning briefing */
...
@media (hover:none){
button,a,.src-row,.trend-row,.today-item,.fc,.pod-card{-webkit-tap-highlight-color:rgba(29,
}
`;
...
const {summary:s,error} = await fetchAISummary({type:'article',title:a.title,content:a.
...
const {summary:t,error} = await fetchAISummary({type:'article',title:a.title,content:a.
...
const r = await fetch(`https://www.reddit.com/search.json?q=${q}&sort=relevance&limit=3
if (r.ok) {
const d = await r.json();
results.reddit = (d?.data?.children||[]).filter(c=>c.data?.num_comments>0).slice(0,3)
...
try {
const q = encodeURIComponent(a.title.slice(0,80));
const r = await fetch(`https://hn.algolia.com/api/v1/search?query=${q}&tags=story&hitsP
if (r.ok) {
const d = await r.json();
...
{topKw && <span className="fc-topic" style={{background:cc.bg,color:cc.color}}>{topKw
<span className="fc-date">{fmtDate(a.pubDate)}</span>
</div>
<div className="fc-body">
{a.img && !imgErr
? <img className="fc-thumb" src={a.img} loading="lazy" onError={()=>setImgErr(true)
...
{loadingAI && !summary
? <div style={{fontSize:'11px',color:'var(--text3)',fontStyle:'italic'}}>Genera
: aiErr && !summary
...
{loadingAI && !takeaways
? <div style={{fontSize:'11px',color:'var(--text3)',fontStyle:'italic'}}>Anal
: aiErr && !takeaways
? <div style={{fontSize:'11px',color:'var(--red)'}}>{aiErr}</div>
...
{loadingDisc
? <div style={{fontSize:'11px',color:'var(--text3)',fontStyle:'italic'}}>Searchin
: (!disc||(disc.reddit.length===0&&disc.hn.length===0))
? <div style={{fontSize:'11px',color:'var(--text3)'}}>No discussions found on R
: <>
{disc.reddit.map((r,i)=>(
<a key={`r${i}`} className="fc-disc-item" href={r.url} target="_blank" re
...
{disc.hn.map((h,i)=>(
<a key={`h${i}`} className="fc-disc-item" href={h.url} target="_blank" re
<span className="fc-disc-platform hn">HN</span>
<span className="fc-disc-title">{h.title}</span>
<span className="fc-disc-stats">{h.points}pts · {h.comments} comments</
</a>
))}
</>}
</div>
...
<button key={i} className="fc-more-src" onClick={()=>s.link&&window.open(s.link,'
))}
</div>
)}
<div className="fc-actions" onClick={e=>e.stopPropagation()}>
<button className={`fc-act ${isSaved?'saved':''}`} onClick={e=>{e.stopPropagation();o
{isSaved?'★ Saved':'☆ Save'}
</button>
<button className={`fc-act ${aiState!=='closed'?'ai-on':''}`} onClick={handleAI} disa
✦ {loadingAI?'Thinking...':aiState==='closed'?'AI Summary':aiState==='summary'?'Key
</button>
<button className={`fc-act ${showDisc?'disc-on':''}`} onClick={handleDisc} disabled={
{loadingDisc?'Searching...':showDisc?'Hide':'Pulse'}
</button>
<a className="fc-read-link" href={a.link} target="_blank" rel="noreferrer" onClick={e
</div>
</div>
);
}
...
const {summary, error} = await fetchAISummary({type:'article',title:a.title,content:a.des
...
{a.img && !imgErr
? <img className="today-thumb" src={a.img} loading="lazy" onError={()=>setImgErr(tr
...
<button className={`today-ai-btn ${showSum?'on':''}`} title="AI Summary" onClick={han
{loading?'…':'✦'}
</button>
</div>
{showSum && (
...
const dateStr = new Date().toLocaleDateString('en-US',{weekday:'long',month:'long',day:'num
const generate = async () => {
setLoading(true); setError('');
// Build a dense context: top 2 headlines per category, unlabelled — let AI synthesize fr
...
content:`Write a single punchy paragraph (3-5 sentences) synthesizing today's biggest s
mode:'summary',
});
...
{text
? <div className="briefing-body"><p className="briefing-overview" dangerouslySetInner
: !loading ? <div className="briefing-loading">Loading articles before generating bri
...
type:'article',
title:`News Overview — ${dateStr}`,
content:`Write a news digest in TWO PARTS.\n\nPart 1: A single punchy 3-4 sentence para
mode:'summary',
});
if (summary) {
...
{body
? <p className="briefing-inline-body" dangerouslySetInnerHTML={{__html: body.replace(
: error
? <p className="briefing-inline-empty">{error}</p>
: <p className="briefing-inline-empty">{loading?'Synthesizing today\'s headlines…':
{bullets.length > 0 && (
<ul className="briefing-inline-bullets">
{bullets.map((b, i) => (
<li key={i} dangerouslySetInnerHTML={{__html: b.replace(/\*\*([^*]+)\*\*/g,'<stro
))}
</ul>
)}
</section>
...
return (
<div key={g.id} className={`sb-game ${isFav?'fav':''} ${live?'live':''}`} onClick={()=>
<div className="sb-game-row">
<div className="sb-side">
{g.awayLogo&&<img className="sb-logo" src={g.awayLogo} alt="" onError={e=>e.targe
<span className="sb-abbr">{g.awayAbbr||g.awayName}</span>
</div>
<span className={`sb-num ${awayWin?'winner':final?'loser':''}`}>{g.awayScore||'—'}<
</div>
<div className="sb-game-row">
<div className="sb-side">
{g.homeLogo&&<img className="sb-logo" src={g.homeLogo} alt="" onError={e=>e.targe
<span className="sb-abbr">{g.homeAbbr||g.homeName}</span>
</div>
<span className={`sb-num ${homeWin?'winner':final?'loser':''}`}>{g.homeScore||'—'}<
</div>
...
return <div className="sb-box"><div className="sb-box-head"><span className="sb-box-title
}
return (
...
? sorted.filter(g=>favoriteIn(g)).concat(sorted.filter(g=>!favoriteIn(g)).slice(0,2
: sorted;
return (
<div key={L.key} className="sb-league">
<button className="sb-league-head" onClick={()=>setExpanded(s=>({...s,[L.key]:!s[
<span style={{color:L.accent,fontWeight:700}}>{L.emoji} {L.label}</span>
<span className="sb-league-meta">
{games.length===0
? <span style={{color:'var(--text3)',fontStyle:'italic'}}>No games today</s
: <><span>{games.length} {games.length===1?'game':'games'}</span>{favCount>
</span>
...
{compact&&sorted.length>visible.length&&<div className="sb-more">+{sorted.len
</div>
)}
</div>
);
...
// ─── GHOST SIDEBAR ────────────────────────────────────────────────────────────
function Sidebar({cat, arts, kw, health, activeKw, setActiveKw, activeSource, setActiveSource
const cc = CATS[cat]||CATS.general;
const catKws = kw[cat]||[];
...
{activeKw && <><span className="gs-filter-label"> {activeKw}</span><button classN
{activeSource && <><span className="gs-filter-label" style={{color:cc.color}}> {a
</div>
)}
...
{sbItems.length === 0
? <div style={{padding:'14px 0',fontSize:'11px',color:'var(--text3)'}}>No stories y
: sbItems.map((a, i) => {
...
{sbTab !== 'trending' && <span className="trend-cat-badge" style={{back
...
{activeKw && <button className="gs-clear" onClick={()=>setActiveKw(null)}>Clear</
<div className="kw-chips">
{catKws.map((k,i)=>(
<span key={i} className={`kw-chip ${activeKw===k?'active':''}`} style={{backgro
))}
</div>
</div>
)}
...
<div className="gs-label">
Sources
{activeSource && <button className="gs-clear" onClick={()=>setActiveSource(null)}
</div>
{sources.map((name,i)=>{
const h=health[name];
const cls=h==='green'?'h-green':h==='yellow'?'h-yellow':h==='red'?'h-red':'h-gray
return (
<div key={i} className={`src-row ${activeSource===name?'active-src':''}`} onCli
<span className={`health-dot ${cls}`} title={h||'Pending'}/>
<span className="src-name" style={{fontWeight:activeSource===name?700:500, co
...
<div className="social-plat-icon" style={{background:meta.color}}>{meta.icon}
<span className="social-plat-label">{meta.label}</span>
<span className="social-plat-count">{handles.length}</span>
</div>
<div className="social-handles">
{handles.map((h,i)=>(
<a key={i} className="social-handle" href={socialUrl(p,h)} target="_blank"
))}
</div>
</div>
);
...
if (!cat) {
const seen=new Set();
Object.values(feeds).flat().forEach(f=>{if(f.on&&!seen.has(f.name)&&SOURCE_URLS[f.name]){
const counts={}; Object.values(arts).flat().forEach(a=>{counts[a.source]=(counts[a.source
...
sources=(feeds[cat]||[]).filter(f=>f.on&&SOURCE_URLS[f.name]).sort((a,b)=>(counts[b.name]
}
...
<a className="src-footer-link" href={SOURCE_URLS[f.name]} target="_blank" rel="no
...
// ─── CUSTOMIZE PANEL ──────────────────────────────────────────────────────────
const CAT_LABELS = {general:' General',sports:' Sports',business:' Business',finance:'
const PLAT_LABELS = {twitter:'𝕏',linkedin:'in',instagram:'IG',youtube:'▶'};
function CustomizePanel({feeds, kw, alerts, urgent, social, watchlist, health, arts, initialT
...
setTestState(s=>({...s,[key]:items.length>0?`ok:${items.length} articles`:`fail:${reason|
};
const addSource = () => {
if (!newName.trim()||!newUrl.trim()) return;
setLf(prev=>{const n=JSON.parse(JSON.stringify(prev));if(!n[srcTab])n[srcTab]=[];n[srcTab
...
setLs(prev=>{const n=JSON.parse(JSON.stringify(prev));n[socCat][socPlat].splice(idx,1);re
};
...
const msg=isLoad?'Testing…':isOk?`✓ ${ts.replace('ok:','')}`:`✗ ${ts.replace('fail:','Fai
return <div className={`cp-test-result ${isLoad?'cp-test-load':isOk?'cp-test-ok':'cp-test
};
return (
<div className="cp-overlay" onClick={onClose}>
<div className="cp-panel" onClick={e=>e.stopPropagation()}>
<div className="cp-head"><span className="cp-title">Customize</span><button className
<div className="cp-body">
<div className="cp-sec-tabs">
{['keywords','alerts','sources','social','watchlist'].map(t=>(
<button key={t} className={`cp-sec-tab ${secTab===t?'active':''}`} onClick={()=
{t==='keywords'?'Keywords':t==='alerts'?'Alerts':t==='sources'?'Sources':t===
</button>
))}
</div>
{secTab==='keywords' && (
...
<div className="cp-desc">Keywords boost matching articles to the top and appear
<div className="cp-cat-tabs">{Object.keys(CAT_LABELS).map(c=><button key={c} cl
<div className="cp-chips">
{(lk[kwTab]||[]).map((k,i)=><span key={i} className="cp-chip cp-chip-kw">{k}<
{(lk[kwTab]||[]).length===0&&<span style={{fontSize:'11px',color:'var(--text3
</div>
<div className="cp-add">
<input className="cp-input" placeholder={`Add ${kwTab} keyword…`} value={newK
onKeyDown={e=>{if(e.key==='Enter'&&newKw.trim()){setLk(p=>{const n={...p};n
<button className="cp-btn" onClick={()=>{if(newKw.trim()){setLk(p=>{const n={
</div>
</div>
)}
Keep s
...
<div className="cp-desc">Red scrolling ticker fires only on these words. <div className="cp-chips">
{lu.map((u,i)=><span key={i} className="cp-chip cp-chip-alert">{u}<button cla
{lu.length===0&&<span style={{fontSize:'11px',color:'var(--text3)'}}>No urgen
</div>
<div className="cp-add">
<input className="cp-input" placeholder="Add urgent word (e.g. hurricane)…" v
onKeyDown={e=>{if(e.key==='Enter'&&newAlert.trim()){setLu(x=>[...x,newAlert
<button className="cp-btn cp-btn-red" onClick={()=>{if(newAlert.trim()){setLu
</div>
<div style={{marginTop:'18px'}}>
<div className="cp-lbl">Keyword Highlights</div>
<div className="cp-desc">Articles matching these show a badge but don't trigg
<div className="cp-chips">
{la.map((a,i)=><span key={i} className="cp-chip cp-chip-kw">{a}<button clas
</div>
</div>
</div>
)}
...
<span className="cp-legend-item"><span className="cp-health cp-h-green"/>Load
<span className="cp-legend-item"><span className="cp-health cp-h-yellow"/>Slo
<span className="cp-legend-item"><span className="cp-health cp-h-red"/>Failed
<span className="cp-legend-item"><span className="cp-health cp-h-gray"/>Pendi
</div>
{(lf[srcTab]||[]).map((f,i)=>{
const h=health[f.name];
const hcls=h==='green'?'cp-h-green':h==='yellow'?'cp-h-yellow':h==='red'?'cp-
...
<button className="cp-test-btn" onClick={()=>testFeed(f.url,tk)}>Test</
<button className={`cp-tog ${f.on?'on':'off'}`} onClick={()=>setLf(prev
<button className="cp-del" onClick={()=>setLf(prev=>{const n=JSON.parse
</div>
<TestResult tkey={tk}/>
</div>
);
...
<div className="cp-add-src-title">+ Add custom source to {CAT_LABELS[srcTab]}
<input className="cp-input-sm" placeholder="Source name" value={newName} onCh
<input className="cp-input-sm" placeholder="RSS URL (https://…)" value={newUr
<div style={{display:'flex',gap:'6px'}}>
<button className="cp-test-btn" style={{flex:1,padding:'5px'}} onClick={()=
<button className="cp-btn" style={{flex:1}} onClick={addSource}>Add Source<
</div>
...
<div className="cp-desc">Accounts you want one-tap access to. These are links —
<div className="cp-cat-tabs">{Object.keys(CAT_LABELS).map(c=><button key={c} cl
<div className="cp-plat-tabs">
{['twitter','linkedin','instagram','youtube'].map(p=>(
<button key={p} className={`cp-plat-tab ${socPlat===p?'active':''}`} {PLAT_LABELS[p]} {SOCIAL_META[p].label}
</button>
onClic
))}
</div>
...
<span key={i} className="cp-chip cp-chip-social">{h}<button className="cp-c
))}
{(ls[socCat]?.[socPlat]||[]).length===0&&<span style={{fontSize:'11px',color:
</div>
<div className="cp-add">
<input className="cp-input" placeholder={socPlat==='linkedin'?'Company or per
onChange={e=>setNewHandle(e.target.value)} onKeyDown={e=>{if(e.key==='Enter
...
<div className="cp-desc">Stocks shown on the Finance page. Click any row {lw.map((w,i)=>(
<div key={i} className="cp-src-row">
<span style={{fontWeight:700,fontFamily:'monospace',color:'var(--accent)',f
<span className="cp-src-name">{w.name}</span>
<button className="cp-del" onClick={()=>setLw(prev=>prev.filter((_,j)=>j!==
</div>
))}
{lw.length===0&&<div style={{fontSize:'11px',color:'var(--text3)',padding:'10px
<div className="cp-add-src" style={{marginTop:'10px'}}>
<div className="cp-add-src-title">+ Add symbol</div>
<input className="cp-input-sm" placeholder="Symbol (e.g. AAPL)" value={newSym
<input className="cp-input-sm" placeholder="Name (e.g. Apple Inc)" value={new
<button className="cp-btn" style={{width:'100%'}} onClick={()=>{
if(newSym.trim()&&newSymName.trim()){setLw(prev=>[...prev,{sym:newSym.trim(
}}>Add to Watchlist</button>
</div>
</div>
)}
<button className="cp-save" onClick={()=>onSave({feeds:lf,kw:lk,alerts:la,urgent:lu
</div>
</div>
</div>
);
...
<span className="menu-sheet-item-emoji">{dark?' ':' '}</span>
<span className="menu-sheet-item-label">{dark?'Light mode':'Dark mode'}</span>
</button>
<button className="menu-sheet-item" onClick={() => { onCustomize(); onClose(); <span className="menu-sheet-item-emoji">⚙</span>
...
{ key:'saved', emoji:'★', label:savedCount>0?`Saved (${savedCount})`:'Saved', active: t
...
text:`${g.awayAbbr||g.awayName} ${g.awayScore} — ${g.homeScore} ${g.homeAbbr||g.homeN
...
<span className="trending-card-badge" style={{background:cc.bg,color:cc.color
</div>
...
<div className="hero-band-badge" style={{background:(CATS[lead._cat]?.color||'#1d
...
<span style={{color:CATS[s._cat]?.color,fontWeight:700}}>{CATS[s._cat]?.label
<span>·</span>
<span>{s.source}</span>
</div>
</div>
...
.filter(a=>a.title&&a.link)
.filter(a=>{const k=a.title.slice(0,60).toLowerCase().replace(/\s+/g,'');if(seen.has(
.sort((a,b)=>new Date(b.pubDate)-new Date(a.pubDate))
.slice(0,6);
}
...
{items.length === 0
? <div style={{padding:'16px 0',fontSize:'11px',color:'var(--text3)'}}>No stories y
: items.map((a, i) => {
const cc = CATS[a.cat] || CATS.general;
return (
<div key={i} className="trend-row" onClick={()=>onRead(a)}>
...
<div className="trend-title">{a.title}</div>
<div className="trend-src">
<span className="trend-cat-badge" style={{background:cc.bg,color:cc.col
{a.source} · {fmtDate(a.pubDate)}
</div>
</div>
</div>
...
const iv=setInterval(()=>{TICKERS.forEach(t=>fetchQuote(t.sym).then(q=>q&&setQuotes(prev=
return ()=>clearInterval(iv);
},[]);
const hasBreaking = breakingItems&&breakingItems.length>0;
const tickerItems = hasBreaking?[...breakingItems,...breakingItems]:[];
const ALL_TABS = ['today','briefing','general','sports','business','finance','bloom','comed
const TAB_LABELS = {today:'Today',briefing:'Briefing',bloom:'Bloom Energy',podcasts:'Podcas
const TAB_CLASS = {general:'t-general',sports:'t-sports',business:'t-business',finance:'t-
...
{wxList.map((wx,i)=>(
<a key={i} className="wx-pill" href={`https://weather.com/weather/today/l/${encod
<span>{wx.emoji}</span>
...
return (
<div key={t.sym} className="ticker-item" onClick={()=>onTickerClick&&onTicker
<span className="ticker-sym" style={{color:t.color}}>{t.sym}</span>
{q
? <><span className="ticker-price">${q.price.toLocaleString('en-US',{mini
<span className={`ticker-chg ${up?'ticker-up':'ticker-down'}`}>{up?'▲
...
<span key={i} className="breaking-item" onClick={()=>item.link&&window.open(i
{item.title} <span style={{opacity:0.6,fontSize:'10px'}}>· {item.source}</s
...
<button className="nav-btn" onClick={()=>setDark(d=>!d)} title="Toggle theme">{da
...
const [alerts, setAlerts] = useState(()=>ld('alerts',['Texans','Astros','Kentucky','Cle
...
const [loading, setLoading] = useState({general:false,sports:false,business:false,finance
...
const wordBoundary = urgent.map(u => new RegExp(`\\b${u.replace(/[.*+?^${}()|[\]\\]/g,'\\
const seen = new Set();
...
(a,cat)=>(kw[cat]||[]).filter(k=>(a.title+(a.desc||'')).toLowerCase().includes(k.toLowerC
[kw]
);
const dedupe = arr=>{const seen=new Set();return arr.filter(a=>{const k=a.title.slice(0,60)
const specificCatKeys = useMemo(()=>{
const keys=new Set();
['sports','business','finance','bloom','comedy'].forEach(c=>{
(arts[c]||[]).forEach(a=>{if(a.link)keys.add(a.link);if(a.title)keys.add(a.title.slice(
});
return keys;
},[arts]);
...
if(activeKw) arr=arr.filter(a=>(a.title+(a.desc||'')).toLowerCase().includes(activeKw.toL
...
arr.sort((a,b)=>{const ka=kwMatch(a,cat).length,kb=kwMatch(b,cat).length;if(kb!==ka)retur
return arr.map(a=>({...a,matchedKw:kwMatch(a,cat),isAlert:urgent.some(u=>(a.title+(a.desc
...
setPodEps(p=>({...p,[pod.name]:items.map(e=>({...e,show:pod.name,host:pod.host,emoji:pod.
...
const results={};
await Promise.allSettled(allSyms.map(async sym=>{const q=await fetchQuote(sym);if(q)resul
setMarketData(prev=>({...prev,...results}));
setMarketLoading(false);
},[watchlist]);
const refreshAll = useCallback(async ()=>{
setArts({general:[],sports:[],business:[],finance:[],bloom:[],comedy:[]});
setLoading({general:false,sports:false,business:false,finance:false,bloom:false,comedy:fa
...
const onRead = a=>{setClicks(c=>({...c,[a.source]:(c[a.source]||0)+1}));window.open(a.link
const onSave = a=>setSaved(s=>s.some(x=>x.link===a.link)?s.filter(x=>x.link!==a.link):[...
...
return(arts[cat]||[]).filter(x=>x.link!==a.link&&matched.some(k=>(x.title+(x.desc||'')).t
};
...
<article className="hero-lead" style={{borderTop:`3px solid ${cc.color}`}} onClic
...
{s.img&&<img className="hero-side-thumb" src={s.img} loading="lazy" onErr
...
{lastUpdated[cat] && <span style={{marginLeft:'10px'}}><LastUpdated timestamp
</span>
<button className="page-customize-btn" onClick={()=>openCustomize('sources',cat
</div>
{(activeKw||activeSrc)&&(
<div style={{display:'flex',gap:'6px',flexWrap:'wrap',marginBottom:'12px'}}>
{activeKw&&<span style={{background:cc.bg,color:cc.color,borderRadius:'20px',
{activeSrc&&<span style={{background:'var(--surface2)',color:'var(--text2)',b
</div>
)}
{isLoading&&!feedItems.length
?<div className="empty-state"><div className="empty-icon">{cc.emoji}</div><div
:feedItems.length===0
?<div className="empty-state"><div className="empty-icon"> </div><div classN
:feedItems.slice(0,20).map((a,i)=><FeedCard key={i} a={a} cat={cat} isSaved={
}
...
<button className="today-section-link" onClick={()=>openCustomize('sources','ge
Customize feeds →
</button>
</div>
...
{/* v20: Bloomberg-style 2px accent bar anchors the section visually <div className="today-section-accent" style={{background:cc.color}}/>
*/}
{loading[cat]
? <div style={{padding:'14px 0',fontSize:'12px',color:'var(--text3)'}}>Lo
: items.length === 0
? <div style={{padding:'14px 0',fontSize:'12px',color:'var(--text3)'}}>
: items.map((a, i) => (
<div key={i} className="gf-item" onClick={()=>onRead(a)}>
{a.img
? <div className="gf-thumb" style={{backgroundImage:`url(${a.im
: <div className="gf-thumb-ph"/>}
<div className="gf-body">
<div className="gf-title">{a.title}</div>
<div className="gf-meta">
<span className="gf-meta-cat" style={{color:cc.color}}>{a.sou
<span>·</span>
<span>{fmtDate(a.pubDate)}</span>
</div>
</div>
...
// ─── PODCASTS PAGE ─────────────────────────────────────────────────────
const PodcastsPage = () => {
const allEps=PODCAST_FEEDS.flatMap(p=>(podEps[p.name]||[]).slice(0,3).map(e=>({...e,show:
const displayEps=activePod?(podEps[activePod.name]||[]).map(e=>({...e,show:activePod.name
...
const{summary:s,error}=await fetchAISummary({type:'podcast',title:ep.title,content:
...
const{summary:t,error}=await fetchAISummary({type:'podcast',title:ep.title,content:
...
<div className="pod-title" onClick={()=>ep.link&&window.open(ep.link,'_blank')}
<div className="pod-meta"><span>{fmtDate(ep.pubDate)}</span>{ep.duration&&<span
...
<div className="fc-summary">
<div className="fc-summary-lbl">✦ AI Summary</div>
{loadPod&&!podSum?<div style={{fontSize:'11px',color:'var(--text3)',fontStyle
:podErr&&!podSum?<div style={{fontSize:'11px',color:'var(--red)'}}>{podErr}</
:<div className="fc-summary-text">{podSum}</div>}
</div>
{podAiState==='takeaways'&&(
<div className="fc-takeaways">
<div className="fc-takeaways-lbl"> Key Takeaways</div>
{loadPod&&!podTake?<div style={{fontSize:'11px',color:'var(--text3)',fontSt
:podErr&&!podTake?<div style={{fontSize:'11px',color:'var(--red)'}}>{podErr
...
<button className="pod-btn" onClick={()=>ep.link&&window.open(ep.link,'_blank')}>
<button className={`pod-btn ${podAiState!=='closed'?'ai-on':''}`} onClick={handle
✦ {loadPod?'Thinking…':podAiState==='closed'?'AI Summary':podAiState==='summary
</button>
<button className={`pod-btn ${sv2?'saved':''}`} onClick={()=>onSave({...ep,link:e
</div>
</div>
);
};
...
<div>
<div className="pod-header-name">{activePod?activePod.name:'All Podcasts'}</d
<div className="pod-header-sub">{activePod?`Hosted by ${activePod.host}`:`${P
</div>
</div>
{displayEps.length===0?<div className="empty-state"><div className="empty-msg">Lo
:displayEps.slice(0,20).map((ep,i)=><PodCard key={i} ep={ep} idx={i}/>)}
</div>
<div className="sidebar">
<div className="pod-shows">
<div style={{fontSize:'10px',fontWeight:'700',color:'var(--text3)',textTransfor
<div className="pod-show-item" onClick={()=>setActivePod(null)}>
<div className="pod-show-emoji"> </div>
<div><div className="pod-show-name" style={{color:!activePod?'#e11d48':''}}>A
{!activePod&&<div className="pod-show-dot"/>}
</div>
{PODCAST_FEEDS.map((p,i)=>{
const eps=podEps[p.name]||[];const latest=eps[0];const isA=activePod?.name===
return (
<div key={i} className="pod-show-item" onClick={()=>setActivePod(isA?null:p
<div className="pod-show-emoji">{p.emoji}</div>
<div style={{flex:1,minWidth:0}}>
<div className="pod-show-name" style={{color:isA?'#e11d48':''}}>{p.name
<div className="pod-show-ep">{podLoading[p.name]?'Loading…':(latest?lat
</div>
...
<div key={i} className="trend-row" onClick={()=>ep.link&&window.open(ep.lin
<div className="trend-num">{i+1}</div>
<div className="trend-body">
...
<div className="page">
{saved.length===0
?<div className="empty-state" style={{paddingTop:'80px'}}><div className="empty-icon"
:<div className="page-grid"><div className="feed-col"><span className="page-header" s
}
</div>
);
// ─── FINANCE PAGE ──────────────────────────────────────────────────────
...
if(isWd&&mins>=570&&mins<960)return{state:'open',label:'Markets Open',color:'var(--gree
if(isWd&&mins>=240&&mins<570)return{state:'pre',label:'Pre-Market',color:'var(--amber)'
if(isWd&&mins>=960&&mins<1200)return{state:'after',label:'After Hours',color:'var(--amb
return{state:'closed',label:'Markets Closed',color:'var(--text3)'};
},[]);
const fmtPrice=n=>n==null?'—':n.toLocaleString('en-US',{minimumFractionDigits:2,maximumFr
...
<span style={{color:marketStatus.color,fontWeight:600}}>{marketStatus.label}<
<span style={{color:'var(--text3)',marginLeft:8}}>· Quotes via Yahoo Finance
</div>
</div>
...
{q?<><span>{up?'▲':'▼'} {fmtChg(q.chg)}</span><span className="fin-index-
</div>
</div>
);
})}
...
<button className="page-customize-btn" onClick={()=>openCustomize('watchlist'
</div>
...
<tbody>
{watchlist.length===0&&<tr><td colSpan={5} className="fin-empty">No symbols
{watchlist.map(w=>{
const q=marketData[w.sym];const up=q&&q.chg>=0;
return (
<tr key={w.sym} onClick={()=>window.open(`https://finance.yahoo.com/quo
<td className="fin-sym">{w.sym}</td>
<td className="fin-name">{w.name}</td>
<td className="fin-px">{q?fmtPrice(q.price):<em style={{color:'var(--
<td className={`fin-px ${up?'fin-up':q?'fin-down':''}`}>{q?fmtChg(q.c
<td className={`fin-px ${up?'fin-up':q?'fin-down':''}`}>
{q&&<span className="fin-pct-pill">{up?'▲':'▼'} {fmtPct(q.pct).repl
</td>
</tr>
);
})}
...
<button className="page-customize-btn" onClick={()=>openCustomize('sources','
</div>
{loading.finance&&!items.length
?<div className="empty-state"><div className="empty-icon"> </div><div :items.length===0
?<div className="empty-state"><div className="empty-icon"> </div><div clas
classN
:items.slice(0,15).map((a,i)=><FeedCard key={i} a={a} cat="finance" isSaved
}
...
activeKw={activeKw} setActiveKw={k=>{setActiveKw(k);setActiveSrc(null);}}
activeSource={activeSrc} setActiveSource={s=>{setActiveSrc(s);setActiveKw(null);}
onRead={onRead} scores={scores} scoresLoading={scoresLoading} showScoreboard={fal
</div>
</div>
);
};
// ─── SOCIAL PAGE ───────────────────────────────────────────────────────
const SocialPage = () => {
const totalAccounts=Object.values(social).reduce((n,cat)=>n+Object.values(cat).reduce((m,
return (
<div className="page">
<div className="social-page-header">
<div>
<div className="social-page-title"> Social Follows</div>
<div className="social-page-sub">{totalAccounts} accounts across {Object.keys(soc
</div>
<button className="page-customize-btn" onClick={()=>openCustomize('social','general
</div>
<div className="social-page-grid">
{Object.keys(CATS).map(cat=>{
const cc=CATS[cat];
const catSocial=social[cat]||{};
const platforms=Object.keys(catSocial).filter(p=>catSocial[p]&&catSocial[p].lengt
const total=platforms.reduce((n,p)=>n+catSocial[p].length,0);
return (
<div key={cat} className="social-cat-block" style={{borderTop:`3px solid <div className="social-cat-head">
<span style={{color:cc.color,fontWeight:700,fontSize:'13px'}}>{cc.emoji} {c
<span style={{fontSize:'10px',color:'var(--text3)'}}>{total} accounts</span
</div>
{platforms.length===0
?<div style={{padding:'16px 14px',fontSize:'11px',color:'var(--text3)',font
:platforms.map(p=>{
const meta=SOCIAL_META[p];const handles=catSocial[p];
return (
<div key={p} className="social-cat-plat">
<div className="social-cat-plat-head">
<div className="social-plat-icon" style={{background:meta.color}}>{
${cc.c
<span className="social-plat-label">{meta.label}</span>
<span className="social-plat-count">{handles.length}</span>
</div>
<div className="social-handles">
{handles.map((h,i)=>(
<a key={i} className="social-handle" href={socialUrl(p,h)} ))}
</div>
</div>
target
);
...
{isMobile && <PtrIndicator distance={ptrDistance} threshold={70} refreshing={refreshi
{tab==='today'&&<TodayPage/>}
