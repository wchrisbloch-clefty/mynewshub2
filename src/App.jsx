...
const STORAGE_KEY = "mynewshub_v14";
const TABS = [
    ...
    <div style={{ minHeight: "100vh", background: "#f1f5f9", fontFamily: "'Lora', serif" }}>
      ...
      <header style={{ background: "#0f172a", padding: "0 32px", position: "sticky", top: 0, zIndex: 100, boxShadow: "0 2px 16px rgba(0,0,0,0.2)" }}>
        <div style={{ maxWidth: "1280px", margin: "0 auto", display: "flex", alignItems: "center", gap: "20px", height: "62px" }}>
          ...
          <nav style={{ display: "flex", gap: "2px", flex: 1, overflowX: "auto" }}>
            {TABS.map(tab => (
              <button key={tab.id} onClick={() => { setActiveTab(tab.id); setSearch(""); }} style={{ background: activeTab === tab.id ? "#1d4ed8" : "transparent", color: activeTab === tab.id ? "#fff" : "#94a3b8", border: "none", borderRadius: "8px", padding: "7px 14px", cursor: "pointer", fontSize: "0.82rem", fontWeight: 600, fontFamily: "'Lora', serif", transition: "all 0.15s", display: "flex", alignItems: "center", gap: "5px", whiteSpace: "nowrap" }}
          ...
          <div style={{ display: "flex", gap: "8px", alignItems: "center", flexShrink: 0 }}>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search…" style={{ padding: "7px 14px", borderRadius: "8px", border: "1px solid #334155", background: "#1e293b", color: "#e2e8f0", fontSize: "0.82rem", fontFamily: "'Lora', serif", outline: "none", width: "150px" }} />
      ...
      <main style={{ maxWidth: "1280px", margin: "0 auto", padding: "28px 32px" }}>
        {loading[activeTab] ? (
          ...
          <div>
            {displayArticles.length > 0 && <div style={{ marginBottom: "16px" }}><span style={{ fontSize: "0.77rem", color: "#94a3b8" }}>{displayArticles.length} articles · refreshed {timeAgo(lastRefresh)}</span></div>}
            {displayArticles.length === 0 && <div style={{ textAlign: "center", padding: "60px 0", color: "#64748b" }}><div style={{ fontSize: "3rem", marginBottom: "12px" }}>📭</div><p style={{ fontFamily: "'Playfair Display', serif", fontSize: "1.05rem", margin: "0 0 6px" }}>No articles found.</p><p style={{ fontSize: "0.85rem", margin: 0 }}>Try refreshing or adjusting your filter.</p></div>}
            <div style={{ columns: "2", columnGap: "18px" }}>
              {displayArticles.map((item, i) => (<div key={i} style={{ breakInside: "avoid" }}><ArticleCard item={item} keywords={tabKeywords} /></div>))}
            </div>
