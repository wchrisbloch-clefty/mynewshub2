...
{(lk[kwTab]||[]).map((k,i)=><span key={i} className="cp-chip cp-chip-kw">{k}<button className=“cp-chip-x” onClick={()=>setLk(p=>{const n={…p};n[kwTab]=n[kwTab].filter((*,j)=>j!==i);return n;})}>✕</button></span>)}
...
<div className="cp-chips">
{la.map((a,i)=><span key={i} className="cp-chip cp-chip-alert">{a}<button className=“cp-chip-x” onClick={()=>setLa(x=>x.filter((*,j)=>j!==i))}>✕</button></span>)}
</div>
<div className="cp-add">
